...
`).join(""),vt=(e,t,r)=>{const s=Fe?Math.floor((new Date-new Date(localStorage.getItem("first_launch_date")||new Date))/6048e5):0,o=Z("modeVacances",!1),a=`
  ...
  `},_t=(e,t={})=>{const{width:r=320,height:s=120,paddingX:o=8,paddingY:a=16}=t,i=r-o*2,c=s-a*2,n=e.length;if(!n)return`<svg viewBox="0 0 ${r} ${s}" class="chart-svg"><text x="${r/2}" y="${s/2}" text-anchor="middle" fill="var(--color-text-secondary)" font-size="12">Aucune donnée</text></svg>`;const l=Math.max(...e.map(y=>y.value),1),u=i/n,v=e.map((y,x)=>{const d=Math.max(y.value/l*c,2),g=o+x*u+u*.15,k=a+c-d,m=u*.7;return`<rect x="${g.toFixed(1)}" y="${k.toFixed(1)}" width="${m.toFixed(1)}" height="${d.toFixed(1)}" rx="2"
    ...
    </rect>`}).join(""),b=e.map((y,x)=>y.label?`<text x="${(o+x*u+u/2).toFixed(1)}" y="${(a+c+12).toFixed(1)}" text-anchor="middle" fill="var(--color-text-secondary)" font-size="8">${y.label}</text>`:"").filter((y,x)=>x%Math.ceil(n/8)===0).join("");return`<svg viewBox="0 0 ${r} ${s+14}" class="chart-svg">${v}${b}</svg>`},Lt=(e,t,r={})=>{const s=pe();if(!s){e.innerHTML=`<div class="screen-inner"><p class="empty-msg">Configure ton profil d'abord.</p></div>`;return}const o=Me(s.dateArret),a=Le(o),i=dt(o),c=ee().sort((d,g)=>new Date(d.created_at)-new Date(g.created_at)),n=I(30),l=n.filter(d=>d.type==="pulsion"),u=[];for(let d=29;d>=0;d--){const g=new Date;g.setDate(g.getDate()-d);const m=n.filter(p=>p.created_at.slice(0,10)===g.toISOString().slice(0,10)).length>0;u.push({x:30-d,y:m?Math.random()*30+55:null,label:g.toLocaleDateString("fr-FR",{day:"numeric",month:"short"})})}const v=Array.from({length:24},(d,g)=>({label:g%3===0?`${g}h`:"",value:l.filter(k=>new Date(k.created_at).getHours()===g).length,color:"var(--color-alert)"})),b=c.filter(d=>d.poids).map((d,g)=>({x:g,y:d.poids,label:`${d.poids} kg — ${new Date(d.created_at).toLocaleDateString("fr-FR")}`})),y=c.filter(d=>d.tourTaille).map((d,g)=>({x:g,y:d.tourTaille,label:`${d.tourTaille} cm`})),x=u.filter(d=>d.y!==null).map(d=>({x:d.x,y:d.y,label:`Score: ${Math.round(d.y)} — ${d.label}`}));e.innerHTML=`
    <div class="screen-inner">
      <header class="screen-header">
